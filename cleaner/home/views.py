...
def gen_frames(request):
    ...
    while True:
        ...
        # print(f"Distance from object to ground in pixels: {distance_in_pixels}")
            cv2.putText(img, f"Distance from object to ground in pixels: {distance_in_pixels}", 
                        (50,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
            final_height = distance_in_pixels

            content.append([distance_in_pixels,contour])

            cv2.rectangle(img, (x, y), (x+w, y+h), (0, 255, 0), 2)

        cv2.line(img, (0, ground_y), (img.shape[1], ground_y), (255, 0, 0), 2)  




        closest_stain = min(content, key=lambda x:x[0])
    ...
    # print(f"Distance from object to ground in pixels: {distance_in_pixels}")
        cv2.putText(img, f"Distance from object to ground in pixels: {distance_in_pixels}", 
                    (150,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
        final_height = distance_in_pixels
...
def home(request):
    ...
    if request.method == "POST":
        ...
        todo = int(todo)
        print(todo)
        return render(request, 'stain.html')
